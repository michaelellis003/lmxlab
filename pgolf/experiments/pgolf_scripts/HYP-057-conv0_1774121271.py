...
class CausalConvMixer(nn.Module):
    """Causal 1D depthwise convolution + gating — replaces attention for local patterns.

    Fully parallel (no sequential scan). Captures n-gram patterns via depthwise conv.
    Gated output: conv(x) * sigmoid(gate(x)) for selective feature passing.
    """
    def __init__(self, dim: int, kernel_size: int = 4, expand: int = 2):
        ...
        self.conv_weight = mx.random.normal((inner, kernel_size, 1)) * 0.02
        self.conv_bias = mx.zeros((inner,))
        self.proj = CastedLinear(inner, dim)
        self.proj.weight = mx.zeros_like(self.proj.weight)  # zero-init output
    ...
    def __call__(self, x: mx.array, **kwargs) -> mx.array:
        # x: (batch, seq, dim)
        h = self.up(x)  # (batch, seq, inner)
        g = mx.sigmoid(self.gate(x))  # (batch, seq, inner)
        # Causal 1D conv: pad left by kernel_size-1
        k = self._kernel_size
        h_pad = mx.pad(h, [(0, 0), (k - 1, 0), (0, 0)])  # (batch, seq+k-1, inner)
        # Depthwise conv via manual sliding window (MLX doesn't have grouped 1d conv)
        # For small kernel (k=4), unrolled sum is fast
        conv_out = self.conv_bias[None, None, :]
        for i in range(k):
            conv_out = conv_out + h_pad[:, i:i + x.shape[1], :] * self.conv_weight[:, i, 0][None, None, :]
        y = conv_out * g  # gated output
        return self.proj(y)
