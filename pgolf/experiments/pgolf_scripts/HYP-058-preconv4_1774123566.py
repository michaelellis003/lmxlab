...
def main() -> None:
    ...
    log(
        ...
        f"linear_weight:{next((b.attn.c_q.weight if hasattr(b.attn, 'c_q') else b.attn.c_q_down.weight for b in model.blocks if hasattr(b.attn, 'c_q') or hasattr(b.attn, 'c_q_down')), next(p for p in model.parameters()['blocks'][0]['attn'].values() if hasattr(p, 'dtype'))).dtype} "
