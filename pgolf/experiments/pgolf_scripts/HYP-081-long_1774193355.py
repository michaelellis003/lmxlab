...
class Hyperparameters:
    ...
    share_qk: bool = bool(int(os.environ.get("SHARE_QK", 0)))  # 0 = separate Q,K weights, 1 = shared (Mahalanobis attention)
    affine_norm: bool = bool(int(os.environ.get("AFFINE_NORM", 0)))  # 0 = RMSNorm (no params), 1 = RMSNorm + learned γ,β
    weight_decay: float = float(os.environ.get("WEIGHT_DECAY", 0.0))  # L2 regularization coefficient (e.g. 0.001)
    random_mlp_fc: bool = bool(int(os.environ.get("RANDOM_MLP_FC", 0)))  # 0 = learned, 1 = freeze fc at random init (random features)
    ortho_random_fc: bool = bool(int(os.environ.get("ORTHO_RANDOM_FC", 0)))  # 0 = Gaussian random, 1 = orthogonal random (better JL)
    prune_pct: float = float(os.environ.get("PRUNE_PCT", 0.0))  # 0 = no pruning, >0 = prune smallest X% of weights before serialization
    pos_mlp_bias: bool = bool(int(os.environ.get("POS_MLP_BIAS", 0)))  # 0 = disabled, 1 = add per-position learnable bias to MLP output
    random_attn_v: bool = bool(int(os.environ.get("RANDOM_ATTN_V", 0)))  # 0 = learned, 1 = freeze V projection at random init
    random_attn_k: bool = bool(int(os.environ.get("RANDOM_ATTN_K", 0)))  # 0 = learned, 1 = freeze K projection at random init
...
class RMSNormWithAffine(nn.Module):
    """RMSNorm + learnable scale (γ) and shift (β). Like LayerNorm but without mean centering."""
    ...
    def __call__(self, x: mx.array) -> mx.array:
        return rms_norm(x) * self.gamma + self.beta


def _make_norm(dim: int, dyt: bool = False, affine: bool = False):
    """Create the right normalization layer."""
    if dyt:
        return DyT(dim)
    if affine:
        return RMSNormWithAffine(dim)
    return RMSNormNoWeight()
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if share_qk and num_heads == num_kv_heads and lowrank_q == 0:
            # Share Q and K projection weights (Mahalanobis attention)
            self.c_k = self.c_q  # same weight tensor
        else:
...
class Block(nn.Module):
    def __init__(
        ...
        pos_mlp_bias: bool = False,
        max_seq_len: int = 1024,
    ):
        super().__init__()
        self.attn_norm = _make_norm(dim, dyt=dyt, affine=affine_norm)
        if not parallel:
            self.mlp_norm = _make_norm(dim, dyt=dyt, affine=affine_norm)
        if pos_mlp_bias:
            self.pos_bias = mx.zeros((max_seq_len, dim), dtype=mx.float32)
        object.__setattr__(self, "_pos_mlp_bias", pos_mlp_bias)
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, lowrank_q=lowrank_q, attn_temp=attn_temp, gated_attn=gated_attn, value_resid=value_resid, xsa=xsa, v_norm=v_norm, pre_conv=pre_conv, share_qk=share_qk)
        if kron_rank > 0:
    ...
    def __call__(self, x: mx.array, x0: mx.array, v_resid: mx.array | None = None, use_xsa: bool | None = None) -> mx.array:
        ...
        if self._parallel:
            ...
            if self._pos_mlp_bias:
                mlp_out = mlp_out + self.pos_bias[:x.shape[1], :].astype(x.dtype)[None, :, :]
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 train_seq_len: int = 1024, share_qk: bool = False,
                 affine_norm: bool = False, pos_mlp_bias: bool = False):
        super().__init__()
        ...
        elif sharing_pattern == "palindrome":
            # Mirror: 0,1,2,2,1,0 — pairs same block across encoder-decoder skips
            forward = list(range(n_unique))
            backward = list(range(n_unique - 1, -1, -1))
            full = forward + backward
            layer_map = [full[i % len(full)] for i in range(num_layers)]
        else:
        ...
        self.final_norm = _make_norm(dim, dyt=dyt, affine=affine_norm)
...
class Muon:
    ...
    def step(self, params: dict[str, mx.array], grads: dict[str, mx.array], step: int, lr_mul: float) -> dict[str, mx.array]:
        ...
        for k in self.keys:
            if k not in grads:
                continue  # frozen param (e.g., random MLP fc)
            p = params[k]
            g = grads[k]
...
def main() -> None:
    ...
    model = GPT(
        ...
        affine_norm=args.affine_norm,
        pos_mlp_bias=args.pos_mlp_bias,
    )
    # Random features: freeze projections at random init (JL lemma + Rahimi & Recht 2007)
    if args.random_mlp_fc:
        for b in model.blocks:
            if hasattr(b, 'mlp') and hasattr(b.mlp, 'fc'):
                if args.ortho_random_fc:
                    # Orthogonal init: optimal for JL (zero correlation between features)
                    import mlx.nn.init as mxi
                    ortho = mxi.orthogonal()
                    b.mlp.fc.weight = ortho(b.mlp.fc.weight)
                    log(f"ortho_random_fc: initialized fc with orthogonal matrix")
                b.mlp.fc.freeze()
                log(f"random_mlp_fc: froze {b.mlp.fc.weight.shape} fc weights")
                break
    if args.random_attn_v:
        for b in model.blocks:
            if hasattr(b, 'attn') and hasattr(b.attn, 'c_v'):
                b.attn.c_v.freeze()
                log(f"random_attn_v: froze {b.attn.c_v.weight.shape} V weights")
                break
    if args.random_attn_k:
        for b in model.blocks:
            if hasattr(b, 'attn') and hasattr(b.attn, 'c_k'):
                b.attn.c_k.freeze()
                log(f"random_attn_k: froze {b.attn.c_k.weight.shape} K weights")
                break
    ...
    while True:
        ...
        # Weight decay: add λ*param to each gradient (decoupled WD)
        if args.weight_decay > 0.0:
            wd = args.weight_decay
            flat_params = dict(tree_flatten(model.parameters()))
            flat_grads = dict(tree_flatten(grads))
            for k in flat_grads:
                if k in flat_params and flat_params[k].ndim >= 2:  # only matrices, not biases/scales
                    flat_grads[k] = flat_grads[k] + wd * flat_params[k].astype(flat_grads[k].dtype)
    ...
    # Post-hoc magnitude pruning (sparse signal processing)
    if args.prune_pct > 0:
        all_mags = []
        for name, arr in flat_state.items():
            if arr.ndim >= 2 and arr.size > 100:
                all_mags.append(mx.abs(arr).reshape(-1))
        if all_mags:
            all_mags = mx.concatenate(all_mags)
            threshold = float(mx.sort(all_mags)[int(len(all_mags) * args.prune_pct / 100)].item())
            pruned = 0
            for name in list(flat_state.keys()):
                arr = flat_state[name]
                if arr.ndim >= 2 and arr.size > 100:
                    mask = mx.abs(arr) > threshold
                    flat_state[name] = arr * mask
                    pruned += int((~mask).sum().item())
            log(f"pruned:{pruned} weights ({args.prune_pct}% threshold={threshold:.6f})")
