...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if lowrank_q > 0:
            # Factor Q as dim→r→dim (PR #215: Q has extreme condition numbers)
        ...
        if attn_temp:
            # Learnable per-head attention temperature (log-space for stability)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        scale = self.scale
        if self._attn_temp:
            # Per-head temperature: scale *= exp(log_temp) per head
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, lowrank_q=lowrank_q, attn_temp=attn_temp)
        if kron_rank > 0:
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        if self._parallel:
            # GPT-J/PaLM style: single norm, parallel attn + MLP
            h = self.attn_norm(x)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, mlp_type=mlp_type, kron_rank=kron_rank, parallel=parallel_block, rot_pairs=mlp_rot_pairs, lowrank_q=lowrank_q, poly_relu=poly_relu, attn_temp=attn_temp)
