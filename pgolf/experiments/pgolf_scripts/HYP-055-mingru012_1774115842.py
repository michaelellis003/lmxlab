...
class MinGRU(nn.Module):
    ...
    def __call__(self, x: mx.array, **kwargs) -> mx.array:
        ...
        # Parallel minGRU via log-space cumulative sum (no sequential loop)
        # h_t = (1-z_t)*h_{t-1} + z_t*h_tilde_t
        # = z_t*h_tilde_t + (1-z_t)*z_{t-1}*h_tilde_{t-1} + ...
        # Using decay coefficients: a_t = log(1-z_t), weighted value: z_t*h_tilde_t
        # The recurrence can be computed as:
        #   log_coeffs = cumsum(log(1-z), axis=1)  (cumulative decay)
        #   h = exp(log_coeffs_t - log_coeffs_k) * z_k * h_tilde_k  summed over k<=t
        # This is equivalent to: h_t = sum_k w_{t,k} * v_k where w_{t,k} = z_k * prod_{j=k+1}^{t} (1-z_j)
        log_1mz = mx.log(mx.maximum(1.0 - z, 1e-6))  # (batch, seq, inner)
        log_cum_decay = mx.cumsum(log_1mz, axis=1)    # cumulative decay in log space

        # Weight for each position: z_k * exp(cum_decay_t - cum_decay_k)
        # = z_k * exp(sum_{j=k+1}^{t} log(1-z_j))
        # Compute weighted values: v_k = z_k * h_tilde_k / exp(cum_decay_k)
        # Then h_t = exp(cum_decay_t) * cumsum(v_k)
        # This is the "log-space trick" for parallel linear recurrences
        log_z = mx.log(mx.maximum(z, 1e-6))
        # Weighted input in log space: log(z_k * |h_tilde_k|) - cum_decay_k
        # But h_tilde can be negative, so we can't use pure log space.
        # Instead, use the direct formulation:
        weighted_input = z * h_tilde * mx.exp(-log_cum_decay)  # de-decay the weighted input
        cum_weighted = mx.cumsum(weighted_input, axis=1)         # cumulative sum
        y = cum_weighted * mx.exp(log_cum_decay)                 # re-apply decay
...
class Block(nn.Module):
    def __init__(
        ...
        if not parallel:
            self.mlp_norm = DyT(dim) if dyt else RMSNormNoWeight()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, lowrank_q=lowrank_q, attn_temp=attn_temp, gated_attn=gated_attn, value_resid=value_resid, xsa=xsa, v_norm=v_norm)
        if kron_rank > 0:
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.blocks = [
            ...
            else Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, mlp_type=mlp_type, kron_rank=kron_rank, parallel=parallel_block, rot_pairs=mlp_rot_pairs, lowrank_q=lowrank_q, poly_relu=poly_relu, attn_temp=attn_temp, gated_attn=gated_attn, value_resid=value_resid, dyt=dyt, xsa=xsa, v_norm=v_norm)
...
def main() -> None:
    ...
    log(
        ...
        f"linear_weight:{(model.blocks[0].attn.c_q.weight if hasattr(model.blocks[0].attn, 'c_q') else (model.blocks[0].attn.c_q_down.weight if hasattr(model.blocks[0].attn, 'c_q_down') else model.blocks[0].attn.linear_z.weight)).dtype} "
