...
class MinGRU(nn.Module):
    ...
    def __call__(self, x: mx.array, **kwargs) -> mx.array:
        ...
        # Parallel scan in log space for h_t = (1-z_t)*h_{t-1} + z_t*h_tilde_t
        # log(1-z) = log-coefficient for h_{t-1}
        # We use the sequential scan (simple, correct) — parallel scan is a GPU optimization
        # At 1024 seq len and 8K batch, sequential is fine on MLX
        log_1mz = mx.log(mx.maximum(1.0 - z, 1e-6))

        # Sequential scan (simple, works on MLX)
        bsz, seqlen, inner = h_tilde.shape
        h = mx.zeros((bsz, 1, inner), dtype=h_tilde.dtype)
        outputs = []
        for t in range(seqlen):
            z_t = z[:, t:t+1, :]
            h = (1.0 - z_t) * h + z_t * h_tilde[:, t:t+1, :]
            outputs.append(h)
        y = mx.concatenate(outputs, axis=1)  # (batch, seq, inner)
