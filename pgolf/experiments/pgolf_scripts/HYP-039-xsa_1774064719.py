...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x: mx.array, v_resid: mx.array | None = None) -> mx.array:
        ...
        if self._xsa:
            ...
            v_norm = mx.sqrt(mx.sum(v_xsa * v_xsa, axis=-1, keepdims=True).clip(min=1e-12))
            v_hat = v_xsa / v_norm
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, lowrank_q=lowrank_q, attn_temp=attn_temp, gated_attn=gated_attn, value_resid=value_resid, xsa=xsa)
        if kron_rank > 0:
    ...
    def __call__(self, x: mx.array, x0: mx.array, v_resid: mx.array | None = None) -> mx.array:
        mix = self.resid_mix.astype(x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        if self._parallel:
            h = self.attn_norm(x)
            attn_out = self.attn(h, v_resid=v_resid)
            mlp_out = self.mlp(h)
            x = x + self.attn_scale.astype(x.dtype)[None, None, :] * attn_out + self.mlp_scale.astype(x.dtype)[None, None, :] * mlp_out
        else:
            attn_out = self.attn(self.attn_norm(x), v_resid=v_resid)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, mlp_type=mlp_type, kron_rank=kron_rank, parallel=parallel_block, rot_pairs=mlp_rot_pairs, lowrank_q=lowrank_q, poly_relu=poly_relu, attn_temp=attn_temp, gated_attn=gated_attn, value_resid=value_resid, dyt=dyt, xsa=xsa)
        ...
        if k > 0 and self.num_encoder_layers >= 2:
            ...
            if self._attn_res and self._attn_res_block_size > 0:
                ...
                for i in range(self.num_encoder_layers):
                    x = self.blocks[self._layer_map[i]](x, x0, v_resid=v_resid)
                    if self._value_resid and v_resid is None:
                        v_resid = self.blocks[self._layer_map[0]].attn._last_v
                    if self.use_skip:
                        skips.append(x)
                for i in range(self.num_decoder_layers):
                    ...
                    x = self.blocks[self._layer_map[self.num_encoder_layers + i]](x, x0, v_resid=v_resid)
