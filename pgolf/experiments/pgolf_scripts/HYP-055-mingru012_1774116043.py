...
class MinGRU(nn.Module):
    ...
    def __call__(self, x: mx.array, **kwargs) -> mx.array:
        ...
        for start in range(0, seqlen, chunk):
            ...
            z_chunk = z[:, start:end, :]           # (bsz, chunk, inner)
            ht_chunk = h_tilde[:, start:end, :]    # (bsz, chunk, inner)
            # Within-chunk: vectorized scan using cumulative decay
            log_1mz = mx.log(mx.maximum(1.0 - z_chunk, 1e-6))
            cum_decay = mx.cumsum(log_1mz, axis=1)  # (bsz, chunk, inner)
            decay = mx.exp(cum_decay)                # (bsz, chunk, inner)
            # h_carry contribution: h * prod(1-z) for each position
            h_carry = h[:, None, :] * decay          # (bsz, chunk, inner)
            # new input contribution: sum of z_k * h_tilde_k * prod_{j=k+1}^{t} (1-z_j)
            inv_decay = mx.exp(-cum_decay)
            weighted = z_chunk * ht_chunk * inv_decay
            cum_new = mx.cumsum(weighted, axis=1) * decay
            chunk_out = h_carry + cum_new            # (bsz, chunk, inner)
            outputs.append(chunk_out)
            # Carry: last hidden state of this chunk
            h = chunk_out[:, -1, :]
