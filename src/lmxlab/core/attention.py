...
@attention_registry.register("gated_gqa")
class GatedGQA(GQA):
    """Grouped-Query Attention with elementwise output gating.

    Applies a sigmoid gate to the attention output before
    the output projection: ``y = y * sigmoid(W_gate @ x)``.
    This is the G1 (elementwise) variant from the gated
    attention paper.

    Inherits QK-norm support from GQA.
    ...
    def __call__(
        ...
        # Elementwise gating: y = y * sigmoid(gate(x))
